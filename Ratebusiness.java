	...
	public static class Map1 extends Mapper<LongWritable, Text, Text, FloatWritable>{
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (mydata.length > 23 && "review".compareTo(mydata[22]) == 0 ){
					...
					context.write(new Text(mydata[2]),new FloatWritable(Float.parseFloat(mydata[20])));
			}	
...
// Driver program
	public static void main(String[] args) throws Exception {
		...
		if (otherArgs.length != 2) {
			System.err.println("Usage: Ratebusiness <in> <out>");
			System.exit(2);
		}
	...
	//	DistributedCache.addCacheFile(new URI("hdfs://cshadoop1"+ otherArgs[1]), conf);       
		...
		Job job = new Job(conf, "Ratebusiness");
		job.setJarByClass(Ratebusiness.class);
...
//		job.setCombinerClass(Reduce.class);
		...
		job2.setJarByClass(Ratebusiness.class);
